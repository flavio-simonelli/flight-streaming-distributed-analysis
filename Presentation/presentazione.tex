...
\begin{frame}{Topologia del Flusso Streaming (DAG)}
    ...
    \begin{tikzpicture}[
        ...
        every node/.style={font=\tiny\sffamily, align=center, rounded corners=3pt, line width=0.5pt, draw=black!70, inner sep=4pt},
        ...
        \node[source] (KafkaSource) {Kafka Source\\(rawStream)};
        \node[filter, right=of KafkaSource] (Watermark) {Watermark Assignment\\(Bounded Out-Of-Orderness)};
        \node[filter, right=of Watermark] (Preprocessing) {Filter: LogicalInconsistencyFilter\\(preprocessingStream)};

        %% BRANCH QUERY 1
        \node[filter, above right=2.3cm and 0.6cm of Preprocessing] (Q1_Filter) {Filter:\\TargetAirlineFilter};
        \node[map, right=of Q1_Filter] (Q1_Map) {Map:\\AirlinePerformanceEvent};
        \node[keyby, right=of Q1_Map] (Q1_KeyBy) {KeyBy:\\airline};
        \node[window, right=of Q1_KeyBy] (Q1_WinAgg) {Window: Tumbling (1h) + Lateness\\+ Aggregate: AirlinePerformanceAggregator};
        \node[sink, right=of Q1_WinAgg] (Q1_Sink) {Kafka Sink:\\q1\_results};

        %% BRANCH QUERY 2
        \node[filter, right=0.8cm of Preprocessing] (Q2_Filter) {Filter:\\ActiveFlightFilter};
        \node[map, right=of Q2_Filter] (Q2_Map) {Map:\\RankAirportsEvent};
        \node[keyby, right=of Q2_Map] (Q2_KeyBy) {KeyBy:\\originAirportId};

        \node[window, right=0.8cm of Q2_KeyBy] (Q2_WAgg6) {Window: Tumbling (6h)\\+ Aggregate (6h)};
        \node[window, above=0.5cm of Q2_WAgg6] (Q2_WAgg1) {Window: Tumbling (1h)\\+ Aggregate (1h)};
        \node[window, below=0.5cm of Q2_WAgg6] (Q2_WAggG) {Window: Global (Trigger: 24h)\\+ Aggregate (Global)};

        \node[keyby, right=of Q2_WAgg1] (Q2_KBy1) {KeyBy:\\windowEnd};
        \node[filter, right=of Q2_KBy1] (Q2_Rank1) {Process: RankProcessor\\(Top 10 Airports)};
        \node[sink, right=of Q2_Rank1] (Q2_Sink1) {Kafka Sink:\\q2\_1h};

        \node[keyby, right=of Q2_WAgg6] (Q2_KBy6) {KeyBy:\\windowEnd};
        \node[filter, right=of Q2_KBy6] (Q2_Rank6) {Process: RankProcessor\\(Top 10 Airports)};
        \node[sink, right=of Q2_Rank6] (Q2_Sink6) {Kafka Sink:\\q2\_6h};

        \node[keyby, right=of Q2_WAggG] (Q2_KByG) {KeyBy:\\windowEnd};
        \node[filter, right=of Q2_KByG] (Q2_RankG) {Process: RankProcessor\\(Top 10 Airports)};
        \node[sink, right=of Q2_RankG] (Q2_SinkG) {Kafka Sink:\\q2\_global};

        %% BRANCH QUERY 3
        \node[filter, below right=2.3cm and 0.6cm of Preprocessing] (Q3_Filter1) {Filter:\\TargetAirlineFilter};
        \node[filter, right=of Q3_Filter1] (Q3_Filter2) {Filter:\\ActiveFlightFilter};
        \node[map, right=of Q3_Filter2] (Q3_Map) {Map:\\DelayDistributionEvent};
        \node[keyby, right=of Q3_Map] (Q3_KeyBy) {KeyBy:\\Tuple2(airline, scheduledHour)};

        \node[window, right=0.8cm of Q3_KeyBy] (Q3_WAgg7d) {Window: Tumbling (7d)\\+ Aggregate (7d) [Percentiles]};
        \node[window, above=0.5cm of Q3_WAgg7d] (Q3_WAgg1d) {Window: Tumbling (1d)\\+ Aggregate (1d) [Percentiles]};
        \node[window, below=0.5cm of Q3_WAgg7d] (Q3_WAggG) {Window: Global (Trigger: 1d)\\+ Aggregate (Global) [Percentiles]};

        \node[sink, right=4.5cm of Q3_WAgg1d] (Q3_Sink1d) {Kafka Sink:\\q3\_1d};
        \node[sink, right=4.5cm of Q3_WAgg7d] (Q3_Sink7d) {Kafka Sink:\\q3\_7d};
        \node[sink, right=4.5cm of Q3_WAggG] (Q3_SinkG) {Kafka Sink:\\q3\_global};
        ...
        \begin{scope}[arrows={-Stealth[scale=0.8]}, line width=0.5pt]
            ...
            %% Q1
            \draw (Preprocessing.east) -- ++(0.3,0) |- (Q1_Filter.west);
            ...
            \draw (Q2_KeyBy.east) -- ++(0.3,0) |- (Q2_WAgg1.west);
            \draw (Q2_KeyBy.east) -- (Q2_WAgg6.west);
            \draw (Q2_KeyBy.east) -- ++(0.3,0) |- (Q2_WAggG.west);
            ...
            %% Q3
            \draw (Preprocessing.east) -- ++(0.3,0) |- (Q3_Filter1.west);
            \draw (Q3_Filter1) -- (Q3_Filter2);
            \draw (Q3_Filter2) -- (Q3_Map);
            \draw (Q3_Map) -- (Q3_KeyBy);
            \draw (Q3_KeyBy.east) -- ++(0.3,0) |- (Q3_WAgg1d.west);
            \draw (Q3_KeyBy.east) -- (Q3_WAgg7d.west);
            \draw (Q3_KeyBy.east) -- ++(0.3,0) |- (Q3_WAggG.west);
