...
\begin{frame}{Topologia Flink: Ingestione e Data Quality}
    ...
    \begin{columns}[c]
        ...
        \begin{column}{0.34\textwidth}
            \begin{block}{Pipeline Comune}
                \scriptsize
                L'ingestione ed il preprocessing sono condivisi tra tutte le query:
                \begin{itemize}
                    \item \textbf{Kafka Source}: Lettura distribuita parallela dal topic \texttt{flights-stream} (parallelismo 6).
                    \item \textbf{Watermark Assignment}: Gestione del tempo logico tramite timestamp estratti dal record con tolleranza \textbf{BoundedOutOfOrderness} (2 min).
                    \item \textbf{Data Quality Filter}: Scarto immediato di record incompleti, con date nulle o valori di ritardo/durata negativi.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Topologia Flink: Focus Query 1}
    ...
    \begin{columns}[c]
        ...
        \begin{column}{0.34\textwidth}
            \begin{block}{Query 1: Airline Performance}
                \scriptsize
                Calcolo della media oraria dei ritardi all'arrivo per le sole compagnie target:
                \begin{itemize}
                    \item \textbf{Filtro Compagnie}: Taglio selettivo per considerare solo le compagnie target indicate.
                    \item \textbf{Partizionamento per Chiave}: Operatore \texttt{keyBy} sul codice \texttt{airline}.
                    \item \textbf{Tumbling Window (1h)}: Finestra temporale ad eventi con tolleranza \textbf{Allowed Lateness} per gestire dati in ritardo, evitando scarti.
                    \item \textbf{Aggregazione}: Implementazione incrementale con \texttt{AggregateFunction} per massimizzare la scalabilità.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Topologia Flink: Focus Query 2}
    ...
    \begin{columns}[c]
        ...
        \begin{column}{0.34\textwidth}
            \begin{block}{Query 2: Airport Ranking}
                \scriptsize
                Identificazione oraria dei primi 10 aeroporti per puntualità delle partenze:
                \begin{itemize}
                    \item \textbf{Filtro Voli}: Inclusione dei soli voli attivi (esclusi voli cancellati o deviati).
                    \item \textbf{Multirisoluzione}: Calcolo parallelo su 3 finestre: Tumbling 1h, Tumbling 6h e Global 24h con trigger custom.
                    \item \textbf{RankProcessor}: Calcolo efficiente del ranking locale basato sul conteggio dei voli puntuali (\texttt{depDelay} $\le$ 15 min) diviso per i voli totali.
                    \item \textbf{Output}: Inviato a tre diversi topic di destinazione.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Topologia Flink: Focus Query 3}
    ...
    \begin{columns}[c]
        ...
        \begin{column}{0.34\textwidth}
            \begin{block}{Query 3: Delay Distribution}
                \scriptsize
                Calcolo dei percentili di ritardo per le partenze negli aeroporti e fasce orarie target:
                \begin{itemize}
                    \item \textbf{Filtri Combinati}: Selezione aeroporti di partenza target e voli attivi.
                    \item \textbf{Chiave Composita}: Partizionamento su \texttt{Tuple2(airline, scheduledHour)}.
                    \item \textbf{Finestre Parallele}: Calcolo contemporaneo su Tumbling 1 giorno, Tumbling 7 giorni e Global Window.
                    \item \textbf{DDSketch e Quantili}: Struttura probabilistica in memoria per calcolare la distribuzione e stimare i percentili ($25^\circ, 50^\circ, 75^\circ$) in tempo lineare.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Parallelismo e Scalabilità: Analisi del Throughput}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            ...
            \begin{minipage}[t][0.43\textheight][t]{\linewidth}
                \centering
                \includegraphics[width=\linewidth]{../Results/charts/throughput/thr_out_trans_comparison}
                \vspace{0.05cm}
                {\tiny Comparazione tra throughput in uscita per i tre scenari}
            \end{minipage}
