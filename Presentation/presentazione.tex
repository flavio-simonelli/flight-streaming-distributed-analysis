...
\SectionFrame[Il simulatore custom in Go per la generazione di flussi accelerati e disordinati ed il buffering distribuito basato su partizionamento Kafka.]{Ingestion e Broker Layer}\begin{frame}{Flusso di Funzionamento del Simulatore Go}
    \centering
    \begin{adjustbox}{max width=0.95\textwidth, max height=0.82\textheight}
    \begin{tikzpicture}[
        ...
        \node[decision, below=0.4cm of Start] (CheckTar) {Archivio .tar.gz\\locale presente?};
        \node[process, left=0.8cm of CheckTar] (DownloadTar) {Download Dataset remoto\\\& Verifica SHA1};
        \node[decision, below=0.4cm of CheckTar] (CheckSHA1) {Verifica SHA1\\dell'archivio valida?};
        
        \node[decision, below=0.4cm of CheckSHA1] (CheckCache) {Cache binaria GOB\\presente?};
        \node[process, below=0.4cm of CheckCache] (Load) {Lettura Record da Cache\\(pre-ordinati)};
        \node[process, right=0.8cm of Load] (ParseTar) {Parsing CSV da Tar.gz\\Ordinamento per Event-Time\\Salvataggio Cache GOB};
        
        \node[process, below=0.4cm of Load] (Schedule) {Calcolo Tempo di Invio (PublishAt)\\e Iniezione Disordine Casuale (OOO)};
        \node[process, below=0.4cm of Schedule] (Sort) {Ordinamento Stabile dei Record\\rispetto a PublishAt};
        
        \node[decision, below=0.4cm of Sort] (LoopCheck) {Record rimasti\\in coda?};
        \node[init, left=1.2cm of LoopCheck] (End) {End: Terminato\\(Chiusura Sink \& Exit)};
        
        \node[decision, below=0.4cm of LoopCheck] (CheckCancel) {Simulazione\\interrotta / annullata?};
        
        \node[process, below=0.4cm of CheckCancel] (CalcWait) {Calcolo attesa residua rispetto a prec.\\(targetWait / SpeedupFactor)};
        
        \node[decision, below=0.5cm of CalcWait] (CheckWait) {Attesa residua\\(targetWait) > 0?};
        \node[process, right=0.8cm of CheckWait] (Wait) {Attesa Temporizzata Ibrida\\(Sleep per attese lunghe,\\Busy Wait per precisione ms)};
        
        \node[sink, below=0.5cm of CheckWait] (Write) {Scrittura Record nel Sink\\(Topic Kafka / Terminal Console)};
        
        \node[process, below=0.4cm of Write] (Update) {Aggiorna contatori e riferimenti\\temporali per il record successivo};
        ...
        \begin{scope}[arrows={-Stealth[scale=0.8]}, line width=0.5pt]
            ...
            \draw (Update.east) -- ++(3.5,0) |- (LoopCheck.east);
        \end{scope}
    \end{tikzpicture}
    \end{adjustbox}
\end{frame}
