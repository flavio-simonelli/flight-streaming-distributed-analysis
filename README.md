...
4. **Hadoop HDFS (3.3.2) / Amazon S3 (State Storage)**: 
   * *In locale*: Flink TaskManager e JobManager si appoggiano a un cluster HDFS locale (`hdfs-master`, `hdfs-worker-1`, `hdfs-worker-2`) per memorizzare in modo distribuito i file di checkpoint. Le librerie di HDFS sono condivise con Flink tramite Docker volume (`hadoop-shared`).
