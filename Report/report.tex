...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{Query 2} (topic \texttt{flights-q2-*h-results}): Partizionato per chiave (\textit{origin\_airport\_id}) per garantire la possibilità ad un consumer di leggere solamente la posizione in classifica di specifici aeroporti.
...
Questo test dimostra la robustezza dell'architettura: il sistema ripristina la corretta elaborazione senza alcuna perdita osservata di eventi né duplicazioni visibili nei risultati. Flink garantisce consistenza \textit{exactly-once} fino dello stato interno; sul tratto Kafka--InfluxDB, configurato \textit{at-least-once}, l'idempotenza delle scritture con la medesima chiave e timestamp di finestra rende innocue le duplicazioni prodotte durante il ripristino. La garanzia end-to-end dipende quindi dal mantenimento di tale schema di chiavi.
