...
L'analisi del throughput a livello di cluster evidenzia differenze significative tra i livelli di parallelismo, in particolare nello scenario a medio carico ($14.400\times$):
\begin{itemize}
    \item \textbf{Parallelismo 1 ($P=1$)}: Come si osserva in Fig.~\ref{fig:cluster_throughput_comparison} (sinistra), il throughput in ingresso reale si attesta intorno a $\approx 2.400$ eventi/secondo. Questo valore è inferiore al throughput nominale di $3.050$ eventi/secondo generato dal simulatore, indicando che un singolo slot di calcolo non è in grado di sostenere il carico complessivo della pipeline, provocando un accumulo di ritardo (\textit{lag}) nei topic di Kafka a monte.
    \item \textbf{Parallelismo 3 e 6 ($P \in \{3, 6\}$)}: Con l'aumento dei thread di elaborazione (Fig.~\ref{fig:cluster_throughput_comparison}, destra), il throughput in ingresso sale e si stabilizza perfettamente alla soglia nominale di $\approx 3.000$ eventi/secondo. Questo dimostra che il cluster ha superato il collo di bottiglia e riesce a consumare i dati alla velocità di generazione del simulatore. Il throughput in uscita totale si attesta a circa $100$ record/secondo complessivi (di cui $\approx 50$ record/s per la Query 1, $\approx 30$ record/s per la Query 2 e $\approx 15-18$ record/s per la Query 3).
\end{itemize}

Nello scenario ad alto carico ($43.200\times$), il throughput reale in ingresso si stabilizza per tutte le configurazioni di parallelismo a circa $\approx 8.100$ eventi/secondo, leggermente al di sotto del valore nominale di $9.150$ eventi/secondo. Questa limitazione non è imputabile a colli di bottiglia computazionali di Apache Flink, bensì a vincoli di I/O di rete o alla capacità di throughput del broker Kafka a singolo nodo nel nostro ambiente AWS EC2. Tuttavia, la latenza end-to-end media (\textit{Latency Mark}) rimane estremamente stabile e controllata al di sotto dei $120$ ms per la Query 2 Global (la più complessa), confermando la stabilità del motore di calcolo sotto carico pesante.
