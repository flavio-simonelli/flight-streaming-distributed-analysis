...
Sky Analytics Stream è una pipeline distribuita per lo stream processing in event-time, progettata per elaborare flussi in tempo reale. L'architettura è organizzata in componenti debolmente accoppiate, ciascuna responsabile di una fase specifica: generazione, brokeraggio, elaborazione, persistenza dello stato, storicizzazione dei risultati e visualizzazione.
...
Come mostrato in Fig.~\ref{fig:architecture_diagram}, il livello di ingestione è basato su Apache Kafka, che agisce da buffer distribuito tra il simulatore e il motore di elaborazione. Kafka permette di assorbire variazioni temporanee del carico, disaccoppiare la velocità di produzione da quella di consumo e conservare i messaggi durante eventuali fasi di recovery. Il livello computazionale è implementato in Apache Flink, che consuma gli eventi dalle partizioni Kafka, assegna timestamp e watermark, mantiene stato per finestre event-time e produce i risultati delle query analitiche. Lo stato locale degli operatori è gestito tramite RocksDB, mentre i checkpoint vengono persistiti su storage distribuito per garantire il ripristino consistente dopo guasto. A valle, InfluxDB memorizza sia i risultati applicativi sia le metriche operative, rese consultabili tramite dashboard Grafana.
...
Il simulatore può anche introdurre disordine temporale artificiale. Con probabilità configurabile, un evento viene ritardato di un offset casuale limitato da una soglia massima espressa in minuti logici; successivamente, la sequenza viene ordinata rispetto al tempo effettivo di pubblicazione. Questo meccanismo permette di produrre stream in cui l'ordine di arrivo non coincide con l'event-time, rendendo possibile valutare sperimentalmente watermark, allowed lateness e gestione dei record tardivi.
