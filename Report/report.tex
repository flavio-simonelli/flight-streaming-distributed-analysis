...
\begin{itemize}
    \item \textbf{Configurazioni A e B}: Registrano un numero di record scartati pari a \textbf{zero} (Fig.~\ref{fig:cluster_delay_comparison_d5}). Nella Configurazione A, il ritardo di $30$ minuti consente a tutti gli eventi fuori ordine di rientrare prima del passaggio del watermark. Nella Configurazione B, sebbene il watermark si muova più rapidamente ($D=5$), la finestra resta attiva in memoria grazie a un' \textit{allowed lateness} di $25$ minuti, garantendo che i voli con ritardo fino a $30$ minuti ($5 + 25$) vengano comunque inclusi nel calcolo finale.
    \item \textbf{Configurazione C}: Mostra una significativa perdita di dati. Nel grafico \textit{Record Scartati per Finestra} (Fig.~\ref{fig:cluster_delay_comparison_d10}, in basso a sinistra), si osserva un accumulo continuo di scarti che raggiunge $\approx 970$ record per la Query 2 (finestra 1h), $\approx 500$ record per la Query 1 e $\approx 250$ per la Query 2 (finestra 6h). La distribuzione dei ritardi degli scarti conferma che tutti gli eventi con sfasamento compreso tra $10$ e $30$ minuti sono stati rigettati da Flink, degradando la precisione dei risultati di business.
\end{itemize}
...
\begin{tabular}{lcc}
\hline
\textbf{Metrica} & \textbf{Intervallo 5 Minuti} & \textbf{Intervallo 30 Secondi} \\
\hline
...
\textbf{TaskManager GC Medio} & \textbf{266 ms} & \textbf{431 ms} \\
\textbf{CPU TaskManager Media} & $\approx 11-13\%$ & $\approx 12-14\%$ \\
...
Nello Scenario 3 viene simulato un guasto critico arrestando brutalmente l'istanza \texttt{flink-worker-3} al sesto minuto di esecuzione reale. Le metriche a livello di cluster (Fig.~\ref{fig:cluster_checkpoint_recovery}) e di nodo (Fig.~\ref{fig:node_checkpoint_recovery}) mostrano la sequenza esatta degli eventi:
\begin{itemize}
...
\subsection{Grafici del Primo Confronto: Parallelismo e Scalabilità}
...
\subsection{Grafici del Secondo Confronto: Gestione del Disordine}

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.48\linewidth]{graphs/cluster_delay_run_d30_l0.png}
    \hfill
    \includegraphics[width=0.48\linewidth]{graphs/cluster_delay_run_d5_l25.png}
    \caption{Throughput e metriche del cluster per la Configurazione A ($D=30$, $L=0$, sinistra) e la Configurazione B ($D=5$, $L=25$, destra).}
    \label{fig:cluster_delay_comparison_d5}
\end{figure*}

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.6\linewidth]{graphs/cluster_delay_run_d10_l0.png}
    \caption{Metriche del cluster per la Configurazione C ($D=10$, $L=0$), con evidenza del grafico ``Record Scartati per Finestra'' in basso a sinistra.}
    \label{fig:cluster_delay_comparison_d10}
\end{figure*}

\subsection{Grafici del Terzo Confronto: Checkpointing e Resilienza}

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{graphs/cluster_chk_run_int5_fail1.png}
    \caption{Telemetria del cluster Flink durante lo Scenario 3 (guasto e ripristino), con evidenza del crollo e successivo picco del throughput (catch-up phase) e della latenza mark.}
    \label{fig:cluster_checkpoint_recovery}
\end{figure*}

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{graphs/node_chk_run_int5_fail1.png}
    \caption{CPU e Memory Utilization dei TaskManager durante lo Scenario 3: il worker 3 subisce il crash, mentre il worker 1 subisce un picco di CPU al $60\%$ per la catch-up phase.}
    \label{fig:node_checkpoint_recovery}
\end{figure*}
