...
\begin{itemize}
    \item \textbf{Partizionamento e Topic:} Il flusso in ingresso sul topic primario \texttt{flights-stream} è configurato con 6 partizioni per consentire a Flink di scalare linearmente fino a parallelismo 6. In questo modo si garantisce un mapping ottimale 1:1 tra partizioni fisiche e subtask del consumer, evitando che subtask rimangano inattivi. I topic di output sono dedicati per singola query e granularità temporale.
    ...
    \begin{itemize}
        ...
        \item \textit{Query 3} (topic \texttt{flights-q3-*-results}): Trasmesso con chiave nulla seguendo quindi una distribuzione bilanciata nativa tramite \textit{Sticky Partitioning} \textit{Round-Robin}.
    \end{itemize}
\end{itemize}
...
Per la stima dei percentili su flussi di dati continui, l'architettura impiega l'algoritmo probabilistico DDSketch. La scelta è supportata dall'analisi comparativa di Fernando et al.~\cite{fernando2023quantiles} (in particolare la Tabella 4), la quale evidenzia come DDSketch garantisca un'elevata accuratezza sia sulle code che sulle porzioni centrali di distribuzioni fortemente sbilanciate (\textit{skewed}), tempi di inserimento e di query minimi (\textit{High Speed}), e una mergeability completa. Questo lo rende preferibile a KLL (che degrada sulle code asimmetriche) o UDDSketch/ReqSketch (caratterizzati da velocità computazionali inferiori).
...
Nel sistema progettato, impostando un'accuratezza relativa $\alpha = 0.01$ e una dimensione massima dello store $m = 2048$ bin (una configurazione che, come evidenziato in~\cite{masson2019ddsketch}, è in grado di coprire un range estremamente ampio che varia da meno di un millisecondo ($80\ \mu\text{s}$) a un anno intero logico coprendo tutti i quantili) tale approssimazione non rappresenta un problema in alcuno dei due scenari di utilizzo:
\begin{enumerate}
    \item \textbf{Query 3 (Distribuzione dei Ritardi dei Voli):} Un'approssimazione dell'1\% su ritardi espressi in minuti (es. $\approx 36$ secondi su un'ora di ritardo) è del tutto irrilevante per le finalità analitiche e statistiche del business aeronautico.
    \item \textbf{Telemetria (Latenze E2E e dell'Operatore):} Nel monitoraggio prestazionale del cluster tramite Grafana, l'accuratezza a livello di millisecondo è ampiamente sufficiente per identificare anomalie e trend, rendendo ininfluente la perdita di precisione microscopica.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Data Quality (\texttt{corrupted\_records\_total})}: Un \texttt{Counter} registrato nell'operatore di preprocessing (\texttt{LogicalInconsistencyFilter}) che rileva e incrementa il numero totale di record scartati poiché non conformi ai vincoli di schema o alle regole di consistenza di business.
    \item \textbf{Record Tardivi (\texttt{LateRecordMetricAnalyzer})}: All'interno della gestione del side-output, vengono registrate metriche per valutare l'accuratezza dei meccanismi di gestione dell'out-of-ordering:
    \begin{itemize}
        ...
        \item \texttt{lateness\_p50/p90/p95/p99\_minutes}: Un gruppo di \texttt{Gauge} che stima l'andamento dei percentili del disallineamento temporale (lateness) servendosi dell'algoritmo DDSketch.
    \end{itemize}
    \item \textbf{Latenze (\texttt{ProcessingLatencyTracker})}: Integrate negli operatori analitici chiave per tracciare i tempi di elaborazione interni e la latenza di transito in millisecondi:
    \begin{itemize}
