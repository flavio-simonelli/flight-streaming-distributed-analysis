...
L'architettura di \textit{Sky Analytics Stream} è concepita come una pipeline multi-tier per lo stream processing distribuito ad alte prestazioni, orientata a garantire elevati standard di scalabilità orizzontale, robustezza strutturale e tolleranza ai guasti end-to-end. Il sistema adotta un approccio a microservizi fortemente disaccoppiati, orchestrati per cooperare in modo asincrono. Al fine di mappare coerentemente questa infrastruttura, la topologia viene analizzata sia sotto il profilo strutturale dello stack tecnologico coinvolto (Fig.~\ref{fig:architecture_diagram}), sia sotto quello dinamico del ciclo di vita operativo dei record (Fig.~\ref{fig:interaction_diagram}).
...
Sotto l'aspetto statico, l'infrastruttura si articola in cinque livelli logici interconnessi. L'\textit{Ingestion Layer} sfrutta le proprietà di un cluster Apache Kafka operante come buffer centralizzato, il cui scopo fondamentale è disaccoppiare la sorgente di generazione dati sia dal motore di calcolo sia dai consumatori a valle, attutendo i picchi improvvisi di traffico ed esercitando una contropressione nativa. Il core computazionale risiede nel \textit{Processing Layer}, dove Apache Flink esegue l'elaborazione stateful ed event-driven modellata come un singolo grafo diretto aciclico. La persistenza dello stato \textit{in-flight} e la resilienza del cluster sono demandate allo \textit{State Storage Layer}, che combina l'efficienza locale di RocksDB (operante su disco/memoria dei worker) con la durevolezza distribuita di HDFS o Amazon S3 per il salvataggio dei checkpoint. A valle, lo \textit{Storage Layer} si affida al time-series database InfluxDB per la storicizzazione efficiente e separata delle metriche di telemetria e dei risultati analitici, i quali vengono infine esposti nel \textit{Visualization Layer} tramite dashboard dinamiche in Grafana per garantire la completa osservabilità del sistema.
...
La traduzione dinamica di questo stack si sviluppa lungo un flusso sequenziale che governa il ciclo di vita di ogni singolo evento di volo, come illustrato nel diagramma di Fig.~\ref{fig:interaction_diagram}. Il processo ha inizio con la fase di \textit{Generazione e Replay}, in cui il simulatore Go estrae i dati grezzi e inietta i payload JSON nei broker. Successivamente, nella fase di \textit{Elaborazione Stateful}, i TaskManager di Flink consumano lo stream parallelamente dalle partizioni di Kafka, gestiscono l'avanzamento dei watermark in event-time e computano le query analitiche concorrenti, re-instradando i risultati intermedi su topic di output dedicati. Durante la marcia ordinaria del job, la fase di \textit{Fault-Tolerance} esegue periodicamente barriere asincrone di checkpoint basate sull'algoritmo di Chandy-Lamport, sincronizzando lo stato locale delle finestre e degli sketch probabilistici su HDFS con consistenza semantica \emph{exactly-once}. Infine, le fasi di \textit{Raccolta, Persistenza e Visualizzazione} vedono l'agente Telegraf consumare asincronamente i record da Kafka per caricarli nei relativi bucket di InfluxDB, mentre il reporter integrato di Flink invia le metriche di telemetria a un canale separato, consentendo a Grafana di aggiornare le dashboard in tempo reale senza interferire con le performance computazionali dello stream engine.
...
L'ingestione e il replay in tempo reale degli eventi storici sono interamente governati da un componente custom sviluppato in linguaggio Go. Questa scelta tecnologica è motivata dalla necessità di disporre di un agente di iniezione ad altissime prestazioni caratterizzato da un footprint di memoria ridotto, capace di azzerare i colli di bottiglia computazionali e prevenire le latenze spurie indotte dai cicli di Garbage Collection tipici degli ecosistemi basati su JVM

Per analizzare accuratamente la resilienza e i limiti operativi della topologia distribuita sotto sforzi differenziati, sono stati configurati tre distinti scenari di carico basati su fattori di accelerazione temporale crescenti, i cui parametri nominali e sperimentali sono dettagliati nella Tabella~\ref{tab:go_acceleration_scenarios}. L'introduzione di tali moltiplicatori comporta una compressione del tempo logico che contrae drasticamente la durata reale delle finestre temporali di Flink, imponendo frequenze di trigger e di calcolo dello stato estremamente elevate.
...
In base alla periodicità dei trigger e al numero di chiavi attive stimate per ciascuna finestra logica, è possibile ricavare analiticamente il throughput massimo teorico e reale atteso in uscita per le tre query analitiche, come riassunto nella Tabella~\ref{tab:max_throughput_expected}. Nella realtà sperimentale, il volume effettivo di output della Query 2 può subire fluttuazioni negative a causa dei filtri stringenti sulle soglie dei ritardi severi, che escludono gli aeroporti a bassa frequenza dalla classifica Top-10. Al contrario, i throughput reali di Query 1 e Query 3 possono evidenziare picchi positivi transitori indotti dalla distributione intrinsecamente non uniforme dei voli reali sull'asse dell'event-time.
...
Per sostenere stabilmente tali regimi di iniezione senza alterare i clock interni di simulazione, l'architettura del generatore sfrutta ottimizzazioni ingegneristiche fondamentali:
\begin{itemize}
    \item \textbf{Ingestione Zero-Copy:} Se non presente localmente, il simulatore scarica il dataset tramite richiesta HTTP e ne valida l'integrità mediante hash SHA-1. L'archivio originale viene scompattato direttamente al volo in memoria RAM, escludendo le operazioni di I/O su disco ed eliminando i tempi morti di bootstrap.
    \item \textbf{Binary Caching:} I record estratti dal file CSV vengono convertiti in strutture tipizzate, ordinati cronologicamente rispetto al loro event-time e serializzati in un file di cache binario locale \texttt{.gob} per minimizzare i tempi di inizializzazione delle esecuzioni successive.
    \item \textbf{Sleep-Spinlock Ibrido:} Per ovviare all'inadeguatezza delle sleep del sistema operativo sotto alti fattori di accelerazione, dove l'overhead dello scheduler e i context switch inficerebbero la precisione del pacing, viene implementata una strategia di attesa mista. Se il delta temporale supera la soglia critica di \textit{spin threshold}, il thread rilascia la CPU; al di sotto di essa effettua un uno spinlock hardware, garantendo la massima accuratezza.
    \item \textbf{Out-of-Order Injection:} Al fine di stressare e validare le metriche di tolleranza ai guasti, le strategie di watermarking e la gestione dei dati tardivi all'interno di Apache Flink, un decoratore probabilistico intercetta ogni record con probabilità $P \in [0, 1]$ e ne trasla l'istante di pubblicazione in avanti di un offset casuale limitato da un tetto configurabile in minuti logici, riproducendo i tipici disallineamenti di rete di uno stream reale.
    \item \textbf{Pubblicazione Asincrona su Kafka:} Il writer adotta una politica di bilanciamento round-robin e accumula i messaggi in micro-batch (fino a 100 messaggi o 100 kB) con un timeout di 2 ms in modalità asincrona, evitando che l'attesa dei singoli ACK di rete distorca la temporizzazione del replay.
\end{itemize}
