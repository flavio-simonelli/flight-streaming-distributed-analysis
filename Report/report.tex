...
\begin{itemize}
    \item \textbf{Throughput in Ingressso e Uscita costante}: A differenza di quanto ci si potrebbe attendere in caso di colli di bottiglia, nello scenario a medio carico ($14.400\times$) il throughput in ingresso reale si attesta costantemente a $\approx 2.928$ eventi/secondo (corrispondente al $96\%$ dell'efficienza reale del simulatore) per tutti i livelli di parallelismo ($P \in \{1, 3, 6\}$). Analogamente, il throughput totale in uscita rimane stabile a $\approx 90$ record/secondo (suddiviso tra Query 1 a $\approx 16$ recs/s, Query 2 a $\approx 28$ recs/s e Query 3 a $\approx 48-50$ recs/s). Questo indica che anche un singolo slot ($P=1$) è sufficiente a processare l'intero carico dello scenario medio senza accumulare lag su Kafka.
