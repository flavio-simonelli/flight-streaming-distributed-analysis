...
Per disaccoppiare la sorgente dei dati dal motore di stream processing ed assorbire le fluttuazioni di carico, è stato integrato Apache Kafka. La componente si articola secondo le seguenti direttive:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Query 3 -- Distribuzione oraria dei Percentili:} Raggruppa gli eventi su una chiave composita formata dalla tupla identificativa \texttt{(airline, hour\_of\_day)} per isolare le 24 fasce orarie della giornata. Lo stream viene elaborato su tre finestre parallele (1 giorno tumbling, 7 giorni sliding e globale). La logica sfrutta una \texttt{ProcessWindowFunction} accoppiata al summenzionato modulo DDSketch come accumulatore di stato incrementale, emettendo i percentili storici interpolati direttamente dai bin dello sketch.
\end{itemize}
